...
const LEARNMORE_TTL = Number(import.meta.env.VITE_LEARNMORE_TTL) || 900000;
...
export function getCached(key: string) {
  const entry = memoryCache[key] || JSON.parse(localStorage.getItem(key) || "null");
...
export function setCached(key: string, value: any, ttlMs?: number) {
  const ttl = ttlMs ?? (key.startsWith("answer:") ? ANSWER_TTL : LEARNMORE_TTL);
  const entry: CacheEntry = { value, expiry: Date.now() + ttl };
